...
from src.parser_en import tag_for_parser, parse_sentence, ParseError
...
try:
    ...
    try:
        ...
        console.print("[yellow]Modelo spaCy 'en_core_web_lg' no encontrado.[/yellow]")
        console.print("Instálalo con:\n  [cyan]python -m spacy download en_core_web_lg[/cyan]")
        NLP = None
except Exception:
    NLP = None
...
def run(
    ...
    for file in files:
        ...
        table = Table("Token", "Etiqueta (POS)")
        for tok in tokens[:-1]:
            table.add_row(tok.word, tok.pos)
        ...
        try:
            ...
            console.print(
                Panel(
                    tree.pretty(),
                    title="Árbol sintáctico (SVO simplificado)",
                    expand=False,
                )
            )
        ...
        if NLP is not None:
            ...
            console.print("\n[bold]Análisis spaCy (modelo en_core_web_lg)[/bold]")
            console.print(t2)
